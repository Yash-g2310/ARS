...
const AssignmentListBlock = ({ subSpaceId, spaceId }) => {
    ...
    const handleAssignmentDetailClick = (assignment_id) => {
        console.log('Assignment clicked');
        console.log(assignment_id)
        navigate(`/spaces/${spaceId}/subspaces/${subSpaceId}/assignments/${assignment_id}/details`);
    }
        ...
        <div className="pl-2 py-2 space-y-1.5">
            {currentAssignmentsList?.map((assignment) => (
                <div
                    ...
                    <button 
                    className="truncate max-w-[calc(100%-2rem)] flex-1" title={assignment.title}
                    onClick={()=>{handleAssignmentDetailClick(assignment.assignment_id)}}
                    >
                        {assignment.title}
                    </button>
                </div>
